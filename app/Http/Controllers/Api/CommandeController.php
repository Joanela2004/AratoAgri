...
class CommandeController extends Controller
{
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'panier.*.quantityKg' => 'required|numeric|min:0.01',
        ]);
        ...
        if ($request->filled('codePromo')) {
            $promotion = Promotion::where('codePromo', $request->codePromo)
                ...
                ->where(function($q) { $q->whereNull('dateDebut')->orWhere('dateDebut', '<=', now()); })
                ->where(function($q) { $q->whereNull('dateFin')->orWhere('dateFin', '>=', now()); })
                ->first();
        ...
        try {
            ...
            foreach ($panier as $item) {
                ...
                DetailCommande::create([
                    ...
                    'poids'        => $poids,
                    'decoupe'      => $item['cuttingOption'] ?? 'entière',
                    'prixUnitaire' => $prixUnitaire,
                    'sousTotal'    => $prixUnitaire * $poids,
                ]);
            }
